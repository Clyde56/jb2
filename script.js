...
 
const API_BASE_URL = 'https://jiaban.2442422196155.workers.dev/';
let userToken = localStorage.getItem('overtime_user_token'); // 从本地存储读取Token
 
    // 状态变量
...
async function login(username, password) {
  ...
  if (response.ok) {
    ...
    userToken = token;
    localStorage.setItem('overtime_user_token', token); // 保存Token到本地
...
 
  ...
  if (!userToken) return;
  // 1. 先更新本地数据模型
  // ... (你原有的更新overtimeData的逻辑)
  // 2. 将整个overtimeData对象发送到后端保存
